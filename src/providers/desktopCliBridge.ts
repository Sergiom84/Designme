...
export function createDesktopCliProvider(options: DesktopCliProviderOptions): Provider {
  async function* generate(req: GenerateRequest): AsyncIterable<GenerateEvent> {
    if (!window.designme?.providerStart || !window.designme?.onProviderEvent || !window.designme?.providerStop) {
      yield { type: 'error', message: options.unavailableMessage };
      return;
    }
    ...
    try {
      const response = await window.designme.providerStart({
        ...
        prompt: options.buildPrompt(req),
      ...
      if (!req.signal.aborted) {
        yield { type: 'error', message: error instanceof Error ? error.message : options.failureMessage };
      }
    } finally {
      unsubscribe();
    }
  ...
  return {
    ...
    async status() {
      ...
      try {
        const result = await window.designme.providerStatus({ providerId: options.id });
